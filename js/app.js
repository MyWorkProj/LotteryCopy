...
$("document").ready(function() {
	...
	if(localStorage.getItem("level")){
		...
		localStorage.setItem("rewardCounts", localStorage.getItem("rewardCounts"));
	}else{
		localStorage.setItem("rewardLevel", rewardLevel);
		localStorage.setItem("rewardCounts", rewardCounts);
	}
	...
	$("body").keydown(function(e) {
		...
		if(e.keyCode == 32 || e.keyCode == 65) {
			var r;
			var crewardLevel = localStorage.getItem("rewardLevel");
			var crewardCounts = localStorage.getItem("rewardCounts");

			//当所有抽奖号全部抽取完毕则销毁跑马和音效循环
			console.log("===zongshu=======" + crewardCounts)
			if(crewardCounts == 8) {
...
//				runingmic.puase();
			...
			if(!isRun) {
				...
				//中奖号处理
				var it = Number(it);
				rewardCounts = parseInt(localStorage.getItem("rewardCounts")) + 1;
				switch(rewardCounts) {
					case 1:
					case 2:
					case 3:
					...
					case 5:
						r = "获得三等奖" ;
						localStorage.setItem("itemCount",130);
					...
					case 7:
						r = "获得二等奖" ;
//						localStorage.setItem("itemCount", parseInt(localStorage.getItem("itemCount"))+ 10);
						localStorage.setItem("itemCount", 140);
		...
		var d = dialog({
			...
			ok: function() {
				if($("#reset:checked").val() && confirm("点击确定将清空抽奖结果。")) {
					...
					localStorage.setItem("rewardCounts",0);
				}
				if($("#personCount").val()) {
				...
				console.log("==1==" + $("#personCount").val());
				console.log("==2==" + $("#itemk").val());
				console.log("===3==" + $("#itemg").val());
